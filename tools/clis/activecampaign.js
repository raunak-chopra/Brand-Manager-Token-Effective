...
function getBaseUrl(rawUrl) {
  ...
  try {
    ...
    console.error(JSON.stringify({ error: 'ACTIVECAMPAIGN_API_URL must be a valid URL' }))
    process.exit(1)
  }
  if (url.protocol !== 'https:') {
    console.error(JSON.stringify({ error: 'ACTIVECAMPAIGN_API_URL must use https' }))
    process.exit(1)
  }
  if (!/^[a-z0-9-]+\.api-(us|eu|ap)\d+\.com$/i.test(url.hostname)) {
    console.error(JSON.stringify({ error: 'ACTIVECAMPAIGN_API_URL host must match youraccount.api-us1.com style hosts' }))
    process.exit(1)
  }
  return `${url.origin}/api/3`
}
...
const MUTATING_COMMANDS = new Set([
  'contacts:create', 'contacts:update', 'contacts:delete', 'contacts:sync',
  'lists:create', 'lists:delete', 'lists:subscribe', 'lists:unsubscribe',
  'deals:create', 'deals:update', 'deals:delete',
  'automations:add-contact',
  'tags:create', 'tags:delete', 'tags:add-to-contact', 'tags:remove-from-contact',
  'webhooks:create', 'webhooks:delete',
])

const SENSITIVE_KEYS = new Set(['email', 'firstName', 'lastName', 'phone', 'first_name', 'last_name', 'phone_number', 'subscriberHash'])
