...
const SENSITIVE_KEYS = new Set(['client_id', 'user_id', 'property', 'name', 'body'])
    ...
    case 'events':
      switch (sub) {
        case 'send': {
          ...
          const apiSecret = GA4_API_SECRET || (args['dry-run'] ? 'dry-run-secret' : undefined)
          if (!apiSecret) { result = { error: 'GA4_API_SECRET environment variable required' }; break }
    ...
    default:
      result = {
        ...
        usage: {
          ...
          events: 'events send --measurement-id <id> --client-id <id> --event-name <name> [--params <json>]',
        }
      }
  }
