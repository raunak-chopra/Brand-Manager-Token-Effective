...
```bash
POST https://www.google-analytics.com/mp/collect?measurement_id={measurement_id}&api_secret=[from GA4_API_SECRET environment variable]
...
Keep the Measurement Protocol API secret out of command history and shared docs. Use `GA4_API_SECRET` in the environment; pass `--api-secret` only for local testing and never paste real values into workspace files.
