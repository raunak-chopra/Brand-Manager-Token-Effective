...
For image-generation workflows, use only static image, display, feed, carousel, story still, thumbnail, and copy limits. Ignore video-specific placements, assets, and generation requirements unless a user asks for copy/spec review only; do not route them into video generation.
